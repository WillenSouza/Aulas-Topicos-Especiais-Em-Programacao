...
def converter(temperatura, de, para):
    ...
    if temperatura is None or isinstance(temperatura, bool) or not isinstance(temperatura, (int, float)):
        print(f"Temperatura inválida: '{temperatura}' não é um número válido")
        return None

    if not isinstance(de, str) or not isinstance(para, str):
        print("Unidades devem ser strings")
        return None
    ...
    if de == "" or para == "":
        print("Unidades não podem ser vazias")
        return None
